...
int APIENTRY WinMain(HINSTANCE instance, HINSTANCE, PSTR, int cmd_show)
{
	...
	while (running)
	{
		...
		for (int i = 0; i < 64; i++)
			mem.AddScatterReadRequest(Handle, listEntry + i * 0x78, &players[i].controller, sizeof(uint64_t));
		mem.ExecuteReadScatter(Handle);

		for (int i = 0; i < 64; i++)
		{
			if (players[i].controller == 0)
				continue;
			mem.AddScatterReadRequest(Handle, players[i].controller + Offsets::m_hPlayerPawn, &players[i].pawnHandle, sizeof(uint64_t));
		}
		mem.ExecuteReadScatter(Handle);

		for (int i = 0; i < 64; i++)
		{
			if (players[i].pawnHandle == 0)
				continue; // skip if invalid
			mem.AddScatterReadRequest(Handle, entityList + 0x8 * ((players[i].pawnHandle & 0x7FFF) >> 9) + 0x10, &players[i].listEntry2, sizeof(uint64_t));
		}
		mem.ExecuteReadScatter(Handle);

		for (int i = 0; i < 64; i++)
		{
			if (players[i].listEntry2 == 0)
				continue;
			mem.AddScatterReadRequest(Handle, players[i].listEntry2 + 0x78 * (players[i].pawnHandle & 0x1FF), &players[i].currentPawn, sizeof(uint64_t));
		}
		mem.ExecuteReadScatter(Handle);

		for (int i = 0; i < 64; i++)
		{
			if (players[i].currentPawn == 0)
				continue;

			mem.AddScatterReadRequest(Handle, players[i].currentPawn + Offsets::m_lifeState, &players[i].lifestate, sizeof(uint64_t));
			mem.AddScatterReadRequest(Handle, players[i].currentPawn + Offsets::m_vOldOrigin, &players[i].enemyPos, sizeof(Vector3));
		}
		mem.ExecuteReadScatter(Handle);
		...
		for (int i = 0; i < 64; i++)
		{
			...
			Vector3 head = {player.enemyPos.x, player.enemyPos.y, player.enemyPos.z + 75.f};

			Vector3 screenPos = player.enemyPos.WTS(View_Matrix);
